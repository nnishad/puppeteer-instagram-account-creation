...
async function likePostsHandler (page) {
  try {
    ...
    for (let item = 1; item <= 100; item++) {
      const xPathStart =
        '/html/body/div[2]/div/div/div[1]/div/div/div/div[1]/div[1]/div[2]/section/main/div[1]/section/div/div[3]/div/div/div[1]/div/article['
...
export const likePost = async (browser, user) => {
  try {
    ...
    const turnOnNotificationButtons = await page.$x(
