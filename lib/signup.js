...
export const signup = async (browser, user, profileId, proxyUsername, proxyPassword) => {
  const page = await browser.newPage()
  await page.authenticate(proxyUsername, proxyPassword)
  ...
  const addAccountDataToProfile = async () => {
    await delay(4000)
    ...
    try {
      ...
      logger.info('Account added to profile' + response)
    } catch (error) {
      logger.error('Failed to add account to profile:' + error)
    }
  }
