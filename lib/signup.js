...
export const signup = async (browser, user, opts) => {
  ...
  const reqDBValues = async () => {
    const getDBAPI = 'http://localhost:3001/profile/unused'
    const response = await fetch(getDBAPI)
    const jsonData = await response.json()
    multiloginProfileId = jsonData.profiles[0].uuid
    remainingCount = jsonData.profiles[0].remainingAccounts
    console.log({ multiloginProfileId, remainingCount })
  }
  await reqDBValues()
  delay(4000)
  ...
  const postData = async (account) => {
    delay(4000)
    // console.log({ multiloginProfileId })
    const url = `http://localhost:3001/profile/${multiloginProfileId}/addAccount`
    const headers = {
      Accept: '*/*',
